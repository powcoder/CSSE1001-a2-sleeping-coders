...
def skipIfFailed(test_case: Type[unittest.TestCase] = None, test_name: Union[str, FunctionType] = None, tag: str = None):
    """
    ...
    """
    if inspect.isfunction(test_name):
...
class TestCase(unittest.TestCase, metaclass=_TestCaseMeta):
    ...
    def assertIsSubclass(self, sub_class: Type, parent_class: Type):
        if not issubclass(sub_class, parent_class):
            self.fail(msg=f"'{sub_class.__qualname__}' should be a subclass of '{parent_class.__qualname__}'")
...
class OrderedTestCase(TestCase):
    ...
    def description(self):
        num = self.member_names.index(self.name) + 1
        pad = ' ' * (3 - len(str(num)))
        return f'{num}.{pad}{super().description}'
